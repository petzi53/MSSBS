...
<!-- ```{theorem, label="title-install-package",  name="Install book packages with dependencies"} -->
<!-- ``` -->
...
3) I will store the full list of loaded packages (N = `r length(.packages())`) into a new generated `_mssbs-data` directory for future reference.

```{theorem, label="title-save-pkg-list",  name="Save a list of all loaded packages of this chapter"}
```
```{r save-pkgs-list}
if (!dir.exists("_data")) dir.create("_mssbs-data")
saveRDS(data.frame(.packages()), file.path("_mssbs-data/pkglist.rds"))
```
...
```{theorem, label="title-cleanup",  name="Detach all non-base packages and clean workspace"}
```

...
```{example,  name="Compare mean, median, trimmed and winsorized mean"}
...
```{lemma,  name="Comparison of mean, median, trimmed and winsorized mean"}
```
...
```{example,  name="Calculation of the ideal fourth"}
...
```{lemma,  name="Function to calculate the ideal fourth"}
```
...
```{lemma,  name="Demonstration data for masking of outliers"}
```
...
```{lemma,  name="outlier function `out(x)`"}
```
...
```{lemma,  name="Demonstrate results of function `out(x)`"}
```
...
```{lemma,  name="How to display the following external image file?"}
```
...
```{lemma,  name="Demonstration of the built-in R function `boxplot(x)`"}
```
...
```{lemma,  name="Outliers values with the boxplot rule using `boxplot` and `boxplot.stats`"}
```
...
```{lemma,  name="Detecting outliers with special funtion `outbox(x)`"}
```
...
```{lemma, label="cmp-outbox",  name="Compare detection of outliers with different bloxplot rules"}
```
...
```{lemma, label="one-step-and-mom-function",  name="Onestep and Modified One-stem M-Estimator (MOM) "}
```
...
```{lemma, label="splot-function",  name="Splot = Frequency Plot with Spikes"}
```
...
```{example, label="rel-freq",  name="Demonstration of the r `hist` function"}
```
...
```{theorem, label="clean-up-2",  name="Detach all non-base packages and clean workspace"}
```
